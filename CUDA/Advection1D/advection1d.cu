/*
This CUDA C program is a 1D linear advection equation solver using finite differences. 
...
{
  if (argc != 2) {
	  cerr << endl<<"Syntax: " << argv[0] << " <Number of points in X (10000-300000) >" << endl;
	return(-1);
	}	
 ...
 // Warm up GPU
  warm_up_gpu<<<(n+BLOCKSIZE-1)/BLOCKSIZE, BLOCKSIZE>>>();
  ...
  checkCuda(cudaMemcpy(d_phi,phi,size,cudaMemcpyHostToDevice));
  ...
  for(int k=0;k<nsteps;k++){       
    ...
    swap(d_phi,d_phi_new); 
         	    
  }
  checkCuda(cudaMemcpy(phi_GPU, d_phi, size, cudaMemcpyDeviceToHost));

  double Tgpu=clock();
  Tgpu=(Tgpu-t1)/CLOCKS_PER_SEC;

  checkCuda(cudaFree(d_phi));
  checkCuda(cudaFree(d_phi_new));
